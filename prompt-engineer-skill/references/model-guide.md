# Model-Specific Prompting Guide (Late-June 2026 Edition)
...
13. [Image Generation (Midjourney V8.1, gpt-image-2, Imagen 4, FLUX.2, Recraft V4.1, Ideogram 3.0)](#image-gen)
14. [Video Generation (Veo 3.1, Gemini Omni, Kling 3.0, Seedance 2.0, Runway Gen-4.5, Grok Imagine 1.5, Wan 2.6)](#video-gen)
15. [Audio Generation (Suno 5.5, ElevenLabs, Lyria 3)](#audio-gen)
16. [Coding Agents (Claude Code/Agent SDK, Cursor, Copilot, Grok Build)](#coding-agents)
17. [Reasoning Models — Critical Differences](#reasoning)
18. [MCP (Model Context Protocol) Prompting under AAIF](#mcp)
19. [Context Engineering & Harness Engineering for Production Systems](#context-eng)
...
**Release:** May 19, 2026 (proprietary, not open-weights)
...
- **Qwen3.7-Max** — May 19, 2026 (current).

### Prompting
- Drop-in compatible with OpenAI/Anthropic API patterns
...
## Image Generation (Midjourney V8.1, gpt-image-2, Imagen 4, FLUX.2, Recraft V4.1, Ideogram 3.0) {#image-gen}
...
**(a) Imagen 4 family (dedicated API / Gemini API / AI Studio):** Imagen 4, **Imagen 4 Ultra** (highest-fidelity, most photorealistic of 2026), and **Imagen 4 Fast** (~$0.02/image, ~2.7s latency). Up to 2K resolution. Strong text rendering.

**(b) Nano Banana (Gemini-native):**
- **Nano Banana Pro** = **Gemini 3 Pro Image** — image gen + editing built on Gemini 3; conversational, advanced world knowledge, subject consistency, improved reasoning over lighting.
- **Nano Banana 2** — combines Pro capabilities with Flash speed (production-ready, subject consistency). This is the consumer-facing "Gemini image generator & photo editor."

**Prompting (Imagen 4):**
- **Short prompts work** — Imagen lets you generate quickly from concise descriptions.
- Supports **negative prompts** (banish unwanted elements) — describe plainly, don't use "no"/"avoid" phrasing in the main prompt; put negatives in the negative-prompt field.
...
### Stable Diffusion (status: SD3.5 still dominant; SD4 Ultra reported but unconfirmed)
**Status:** Stability AI's last *officially confirmed* family remains **SD 3.5** (Large 8B, Large Turbo 8B 4-step, Medium 2.5B, Oct 2024). However, **Stable Diffusion 4 Ultra** is reported in 2026 as "a major architectural overhaul built on an upgraded diffusion transformer (DiT)" focused on photorealism. ⚠️ This rests mainly on secondary sources — treat as "reported, verify."

**Reality on the ground (2026):** The open-source community still runs heavily on **SDXL + fine-tunes/LoRAs**, **SD3.5**, and **FLUX** models.
...
### Ideogram / Recraft / niche
**Ideogram 3.0 (V3):** Widely recognized for **legible text-in-image** rendering — best for typographic designs, signage, packaging, brand campaigns. Photorealistic images + precise style control in a single generation. ~10 credits/image (good value).

**Recraft V4 → V4.1:** "Design taste meets image generation." Two tiers — **photorealistic** and **native SVG vector**. Art-directed composition, accurate text, and **real editable SVGs** (V4.1 Vector). Built for logos, packaging, agency-quality brand assets.
...
| Text-in-image, layouts, UI mockups, infographics | **gpt-image-2** / **Ideogram 3.0** |
| Photorealistic stills (API) | **Imagen 4 Ultra** / **FLUX.2 [max]** |
| Fast/cheap open-weight images | **FLUX.2 [klein]** / **SDXL+LoRA** |
| Editable vectors / brand assets | **Recraft V4.1 Vector** |
| Conversational image editing | **Nano Banana 2 (Gemini)** / **gpt-image-2** |

---

## Video Generation (Veo 3.1, Gemini Omni, Kling 3.0, Seedance 2.0, Runway Gen-4.5, Grok Imagine 1.5, Wan 2.6) {#video-gen}
...
| Physical-realism motion | **Seedance 2.0** |
| Open-source video | **Wan 2.6** |
| Image-to-video + native audio, less censorship | **Grok Imagine 1.5** |
| Reference-driven consistent video | **Runway Gen-4.5** |

---

## Audio Generation (Suno 5.5, ElevenLabs, Lyria 3) {#audio-gen}

- **Suno V5 / 5.5** — **Suno 5.5 released March 27, 2026**: studio-grade results ~10× faster than prior versions, custom voices/models, and a **Studio DAW**. V5 brought human-like vocals; generally the "best overall" pick. (Free tier ~50 credits/day.)
- **ElevenLabs Music** — most generous free tier in 2026 (~7 full songs/day); also the leader for **voice/voice cloning** (v3-class voice models). Strong for streaming/content/commercial use.
- **Google Lyria 3** — Google's music model, in the 2026 comparison set.
- **Udio** — still a contender; compared head-to-head with Suno on audio quality and copyright safety.
- **MiniMax music** and **Stable Audio** — also in the 2026 mix.

**Prompting audio:** Treat music prompts like image prompts — concrete musical facts (BPM, key, instruments, reference artists, decade, production style) beat vague descriptors ("upbeat", "energetic"). For voice cloning, provide clean reference audio + target script with explicit pacing/emotion markers.
...
Key changes:
- **Stateless protocol core**: the `initialize` handshake and session id are removed — any MCP request can now hit any server instance (huge for horizontal scaling / load balancing).
...
An **MCP Registry (Q4 2026)** is planned as a curated, verified, security-audited directory.
...
## Context Engineering & Harness Engineering for Production Systems {#context-eng}

### The 2026 paradigm shift

**Prompt engineering (2022) → Context engineering (2024–25) → Harness engineering (2026)**

The dominant 2026 narrative (per the "From Prompts to Harnesses" retrospective) is a three-stage paradigm shift. **Harness engineering** was coined by Andrej Karpathy in February 2026 (declaring the "vibe coding" era effectively over and introducing "agentic engineering" as the next evolution). In May 2026, Karpathy joined Anthropic to work on large-scale model training for the Claude family.

**The context window = RAM for the LLM.** What you put in determines what the model can do. The harness = prompt + context + tools + memory + runtime.
